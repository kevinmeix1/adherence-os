...
| Tested-action ranking | Options ordered by scenario-score change under explicit feature assumptions when every marginal feature bound passes. | Rankings are disabled when a bound is exceeded and do not estimate treatment effects or prescribe care. |
| Marginal feature-bounds gate | Checks each feature independently against a training-only percentile range or valid binary set before patient-specific ML evidence is shown. | It does not detect implausible feature combinations, semantic drift, clinical safety, or real-world validity. |
| Deterministic safety override | Fixed rules that can suppress every tested action and prepare a local review draft or urgent route independently of ML or generated text. | The prototype rules require clinical governance before real deployment. |
