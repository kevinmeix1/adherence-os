...
| Adherence Twin | A patient-specific presentation of current drivers, protective factors, and bounded support options. | Its failure-point text is heuristic; it is not a biological digital twin or disease simulation. |
...
| Route ranking | Supported options ordered by scenario-score change under explicit feature assumptions. | Rankings are disabled outside training support and do not estimate treatment effects or prescribe care. |
